...
class JoyToTwist(Node):
    ...
    def joy_callback(self, msg):
        ...
        left_motor_magnitude = 0.0
        right_motor_magnitude = 0.0

        left_right = msg.axes[6]
        self.logger.info(str(left_right))

        up_down = msg.axes[7]
        self.logger.info(str(up_down))

        # If up > 0.95 fwd
        self.logger.info(f'up_down: {up_down}')
        if (up_down > 0.95):
            left_motor_magnitude = 0.5
            right_motor_magnitude = 0.5

        # If up < -0.95 rev
        if (up_down < -0.95):
            self.logger.info(f'HERE! {up_down}')
            left_motor_magnitude = -0.5
            right_motor_magnitude = -0.5
        ...
        if (up_down < 0.05 and up_down > -0.05 and left_right < 0.05 and left_right > -0.05):
            left_motor_magnitude = 0.0
            right_motor_magnitude = 0.0

        self.logger.info(f"Left motor: {left_motor_magnitude}")
        self.logger.info(f"Right motor: {right_motor_magnitude}")
